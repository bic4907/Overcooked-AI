...
from model.DDPGAgent import DDPGREMAgent
...
class MADDPG(object):
    ...
    def __init__(self, name, params):
        ...
        self.agents = [DDPGREMAgent(params) for _ in range(self.num_agents)]
    ...
    def update(self, replay_buffer, logger, step):
        ...
        for agent_i, agent in enumerate(self.agents):
            ...
            ''' REM Code '''
            alpha = torch.rand((self.num_heads, 1))  # , requires_grad=True)
            ...
            with torch.no_grad():
                ...
                target_critic_out = agent.target_critic(target_critic_in)

                ''' REM Regularization '''
                regularized_value_target = torch.matmul(target_critic_out, alpha)

                target_next_q = rewards[:, agent_i] + (1 - dones[:, agent_i]) * self.gamma * regularized_value_target

            critic_in = torch.cat((obses, actions), dim=2).view(self.batch_size, -1)
            main_q = agent.critic(critic_in)

            ''' REM Regularization '''
            regularized_main_value = torch.matmul(main_q, alpha)

            critic_loss = self.mse_loss(regularized_main_value, target_next_q)
            ...
            critic_out = agent.critic(critic_in)

            ''' REM Regularization '''
            regularized_critic_value = torch.matmul(critic_out, alpha)

            actor_loss = -regularized_critic_value.mean()
