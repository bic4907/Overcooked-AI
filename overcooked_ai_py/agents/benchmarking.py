...
class AgentEvaluator(object):
    ...
    def evaluate_agent_pair(self, agent_pair, num_games, game_length=None, start_state_fn=None, metadata_fn=None, metadata_info_fn=None, display=False, dir=None,
        ...
        # this index has to be 0 because the Agent_Evaluator only has 1 env_particle initiated
        # if you would like to evaluate on a different env_particle using rllib, please modifiy
        # rllib/ -> rllib.py -> get_rllib_eval_function -> _evaluate

        # native eval: using self.env_particle in evaluation instead of creating a copy
        # this is particulally helpful with variable MDP, where we want to make sure
        # the mdp used in evaluation is the same as the native self.env_particle.mdp
        if native_eval:
