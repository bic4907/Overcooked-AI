...
class OvercookedState(object):
    ...
    def from_dict(state_dict):
        ...
        object_list = []
        for key in state_dict["objects"]:
            o = state_dict["objects"][key]
            object_list.append(SoupState.from_dict(o))
...
class OvercookedGridworld(object):
    ...
    def __init__(self, terrain, start_player_positions, start_bonus_orders=[], rew_shaping_params=None,
                 layout_name="unnamed_layout", start_all_orders=[], num_items_for_soup=3, order_bonus=2,
                 start_state=None, **kwargs):
        """
    ...
    def featurize_state(self, overcooked_state, mlam, num_pots=2, **kwargs):
        ...
        for i, player in enumerate(overcooked_state.players):
            ...
            all_features = concat_dicts(all_features, make_closest_feature(i, player, "onion",
                                                                           self.get_onion_dispenser_locations() +
                                                                           counter_objects["onion"]))
            all_features = concat_dicts(all_features, make_closest_feature(i, player, "tomato",
                                                                           self.get_tomato_dispenser_locations() +
                                                                           counter_objects["tomato"]))
            all_features = concat_dicts(all_features, make_closest_feature(i, player, "dish",
            ...
            all_features = concat_dicts(all_features,
                                        make_closest_feature(i, player, "serving", self.get_serving_locations()))
            all_features = concat_dicts(all_features, make_closest_feature(i, player, "empty_counter",
                                                                           self.get_empty_counter_locations(
                                                                               overcooked_state)))

            # Closest pots info
            pot_locations = self.get_pot_locations().copy()
            for pot_idx in range(num_pots):
                _, closest_pot_loc = mlam.motion_planner.min_cost_to_feature(player.pos_and_or, pot_locations,
                                                                             with_argmin=True)
        ...
        for i, player_i in enumerate(overcooked_state.players):
            ...
            player_i_ordered_features = np.squeeze(
                np.concatenate([player_i_features, other_player_features, player_i_rel_pos, player_i_abs_pos]))
